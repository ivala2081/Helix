// V6.2 paper-test warmup: initializes 5 live_v6_2_portfolios rows by feeding
// 150 historical candles through stepCandle() with V6_2_DEFAULTS.
//
// Run once after the migration creates live_v6_2_* tables, before the cron
// starts ticking V6.2 portfolios.
//
// Usage: npx tsx scripts/warmup_v6_2.ts
...
async function main() {
  ...
  for (const symbol of COINS) {
    ...
    for (const candle of candles) {
      stepCandle(state, candle, V6_2_DEFAULTS);
    }
    ...
    console.log(`  + ${symbol} V6.2 portfolio warmed up and persisted`);
  }

  console.log("\n── V6.2 warmup complete ──");
}
...
async function fetchCandles(symbol: string, count: number): Promise<Candle[]> {
  ...
  return raw
    .filter((row) => Number(row[0]) < currentHourStart)
    .map((row) => ({
