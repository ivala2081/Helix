...
import type { ExitReason, Metrics } from "@/lib/engine/types";
...
export function ExitReasons({ metrics }: { metrics: Metrics }) {
  ...
  const data = ORDER.filter((k) => (metrics.exitReasons[k] ?? 0) > 0).map((k) => ({
    name: k,
    value: metrics.exitReasons[k] ?? 0,
  }));
            ...
            <PieChart>
              ...
              >
                {data.map((entry) => (
                  <Cell
                    key={entry.name}
                    fill={COLORS[entry.name as ExitReason] ?? "#71717a"}
                  />
                ))}
              </Pie>
              <Tooltip
            ...
            <div key={d.name} className="flex items-center justify-between gap-2">
              <span className="flex items-center gap-1.5 text-[var(--color-muted)]">
                <span
                  ...
                  style={{ backgroundColor: COLORS[d.name as ExitReason] }}
                />
                {d.name}
              </span>
